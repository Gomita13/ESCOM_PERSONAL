...
void main(int argc, char *argv[]){
	...
	for(i=0;i<NO_MAT;i++){
		...
		if(ids[i] == -1){
			perror("(hijoMulti.c) An error ocurred during shmget():\n");
			exit(1);
		}
		//Obtengo acceso a la memoria, soy admin
		mats[i] = shmat(ids[i],NULL,0);
		if(mats[i] == (char **) -1){
			perror("(hijoMulti.c) An error ocurred during shmat():\n");
			exit(1);
		}
	}
	...
	if((ids[NO_MAT] = semget(keys[NO_MAT],1,0)) == -1){
		perror("(hijoMulti.c) An error ocurred during semget():\n");
		exit(1);
	}
	...
	//Como es el nieto y se ejecuta primero, bloquea el semaforo
	if(semop(ids[NO_MAT],&bf,1) == -1){
		printf("(hijoMulti.c) An error ocurred during semop(lock)\n");
		exit(-1);
	}
	...
	//Le paso el control al hijo desbloqueando el semaforo
	bf.sem_op = 1;

	if(semop(ids[NO_MAT],&bf,1) == -1){
		printf("(hijoMulti.c) An error ocurred during semop(unlock)\n");
		exit(-1);
	}
