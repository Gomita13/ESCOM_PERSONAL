...
void main(){
	...
	key_t keys[NO_MAT+1] = {5510,5511,5512,5598}; //Matrices A(10),B(11),C(12),semaforoPH(98)
	...
	for(i=0;i<NO_MAT;i++){
		...
		if(ids[i] == -1){
			perror("(padre.c) An error ocurred during shmget():\n");
			exit(1);
		}
		//Obtengo acceso a la memoria, soy admin
		mats[i] = shmat(ids[i],NULL,0);
		if(mats[i] == (char **) -1){
			perror("(padre.c) An error ocurred during shmat():\n");
			exit(1);
		}
	}

	//Creo el semaforo PH
	if((ids[NO_MAT] = semget(keys[NO_MAT],1,IPC_CREAT | 0666)) == -1){
		printf("(padre.c) An error ocurred during semget(HN)\n");
		exit(-1);	
	}

	//Libero el semaforoHN
	if(semop(ids[NO_MAT],&bf,1) == -1){
		printf("(padre.c) An error ocurred during semop(HN-unlock)\n");
		exit(-1);
	}
	...
	idChild = fork();
	if(idChild < 0){
		perror("(padre.c) An error ocurred during child creation:\n");
		exit(1);
	}
	...
	if(idChild == 0){ //Es el hijo
		...
		if(idCChild > 0){ //Es el hijo
			...
			perror("(padre.hijoSuma.c) An error ocurred during child creation:\n");
			exit(1);		
		}
	}else{//Es el padre
		...
		if(semop(ids[NO_MAT],&bf,1) == -1){
			printf("(padre.c) An error ocurred during semop(PH-lock)\n");
			exit(-1);
		}
